...
function useForm(initialState) {
    var _a = react.useReducer(formReducer, __assign({}, getState(initialState))), formState = _a[0], dispatch = _a[1];
    var setFormState = react.useCallback(function (state) {
        dispatch({
            type: FormAction.SET_FORM,
            payload: { state: __assign({}, getState(state)), value: '', id: '' }
        });
    }, []);
    var onTouchHandler = react.useCallback(function (event) {
